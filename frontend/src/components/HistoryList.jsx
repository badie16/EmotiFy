  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-md">
        {error}
        <button onClick={fetchHistory} className="ml-4 text-red-600 underline">
          Réessayer
        </button>
      </div>
    )
  ...
  if (history.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/" className="bg-indigo-500 text-white px-6 py-2 rounded-md hover:bg-indigo-600 transition-colors">
          Faire une analyse
        </Link>
      </div>
    )
            ...
            <div key={item.id} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
              ...
              <div className="mt-4 flex items-center">
                ...
                <span
                  ...
                  style={{
                    backgroundColor: `rgba(${dominant.name === "joy" ? "16, 185, 129" : dominant.name === "sadness" ? "59, 130, 246" : "239, 68, 68"}, 0.1)`,
                  }}
                >
