    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="h-80">
        {chartType === "pie" ? (
            ...
            <PieChart>
              ...
              >
                {filteredData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={emotionColors[entry.name] || "#" + ((Math.random() * 0xffffff) << 0).toString(16)}
                  />
                ))}
              </Pie>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={filteredData} layout="vertical" margin={{ top: 5, right: 30, left: 80, bottom: 5 }}>
        ...
        {filteredData.map((emotion) => (
          <div key={emotion.name} className="flex items-center">
            <div
              ...
              style={{
                backgroundColor: emotionColors[emotion.name] || "#" + ((Math.random() * 0xffffff) << 0).toString(16),
              }}
            ></div>
